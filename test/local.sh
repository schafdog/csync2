...
function cmd {
    ...
    echo "select filename from file order by filename; select peername,filename,operation,other,op from dirty order by op, filename, peername;" | ./connect_${DATABASE}.sh > ${TESTNAME}/${COUNT}.${DATABASE} 2> /dev/null
...
function clean {
    ...
    echo "delete from dirty where myname like '%' ; delete from file where hostname like '%'" | ./connect_${DATABASE}.sh > ${TESTNAME}/${COUNT}.${DATABASE} 2> /dev/null
