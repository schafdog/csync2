...
int db_sql_check_file(db_conn_p db, const char *file, const char *encoded,
		char **other, char *checktxt, struct stat *file_stat, BUF_P buffer,
		int *operation, char **digest, int ignore_flags, dev_t *old_no) {
	int db_flags = 0;
	...
	SQL_BEGIN(db, "Checking File",
			...
			"AND filename = '%s' ", myhostname, encoded)
...
int db_sql_list_dirty(db_conn_p db, char **active_peers, const char *realname,
		int recursive) {
	//unused
	(void) recursive;
...
int db_sql_upgrade_db(db_conn_p db) {
	...
	for (p = csync_prefix; p; p = p->next) {
		if (p->name && p->path) {
			...
			csync_info(1, "Replace prefix %s with path %s (%s)", prefix_encoded,
					p->path, path_encoded);
...
char* csync_generate_recursive_sql(const char *file_encoded, int recursive,
		int prepend_where, int append_and) {
	char *where_rec;
	...
	if (recursive) {
		if (file_encoded == NULL || !strcmp(file_encoded, "/"))
			...
			ASPRINTF(&where_rec,
					"%s (filename = '%s' OR filename LIKE '%s/%%') %s", where,
					file_encoded, file_encoded, and_sql);
		}
	} else {
		if (file_encoded != NULL)
			ASPRINTF(&where_rec, "%s filename = '%s' %s", where, file_encoded,
					and_sql);
		else
			// Also recursive
			ASPRINTF(&where_rec, "");
...
// No longer in use
int db_sql_update_format_v1_v2(db_conn_p db, const char *file, int recursive,
		int do_it) {
	textlist_p tl = 0, t;
...
void db_sql_force(db_conn_p db, const char *realname, int recursive) {
	char *where_rec = csync_generate_recursive_sql(db_escape(db, realname),
			recursive, 0, 1);
	SQL(db, "Mark file as to be forced",
			"UPDATE dirty SET forced = 1 WHERE %s myname = '%s'", where_rec,
			myhostname);
...
void db_sql_mark(db_conn_p db, char *active_peerlist, const char *realname,
	...
	csync_mark(db, realname, NULL, active_peerlist, OP_MARK, checktxt, device,
			inode, mode, mtime);

	if (recursive) {
		textlist_p tl = 0;
		// Missing marking files only in FS
		char *where_rec = csync_generate_recursive_sql(db_encoded, recursive, 0,
				1);
...
{				const char *filename = SQL_V(0);
		...
		while (tl != 0) {
			...
			csync_mark(db, filename, NULL, active_peerlist, OP_MARK, checktxt,
					device, inode, mode, mtime);
			tl = tl->next;
		}
		textlist_free(ptr);
	}
...
// Used by insynctest
textlist_p db_sql_list_file(db_conn_p db, filename_p filename,
		const char *myhostname, peername_p peername, int recursive) {
...
void db_sql_list_sync(db_conn_p db, const char *myhostname,
		const char *peername) {
	SQL_BEGIN(db, "DB Dump - File",
...
textlist_p db_sql_get_command_filename(db_conn_p db, const char *command,
		const char *logfile) {
	textlist_p tl = 0;
	SQL_BEGIN(db,
...
void db_sql_remove_dirty(db_conn_p db, peername_p peername, filename_p filename,
		int recursive) {
	const char *file_enc = db_escape(db, filename);
...
textlist_p db_sql_find_dirty(db_conn_p db,
		int (*filter)(filename_p filename, const char *localname,
				peername_p peername)) {
	textlist_p tl = 0;
...
void db_sql_add_dirty_simple(db_conn_p db, const char *myhostname,
		peername_p peername, filename_p filename) {
	SQL(db, "Add operation",
			"INSERT INTO dirty (myname, peername, filename) values ('%s', '%s', '%s')",
			db_escape(db, myhostname), db_escape(db, peername),
			db_escape(db, filename));
}
void db_sql_clear_operation(db_conn_p db, const char *myhostname,
		peername_p peername, filename_p filename /* , int recursive */) {
...
textlist_p db_sql_get_dirty_by_peer_match(db_conn_p db, const char *myhostname,
		peername_p peername, int recursive, const char *patlist[], int numpat,
		int (*get_dirty_by_peer)(filename_p filename, const char *pattern,
				int recursive)) {
	textlist_p tl = 0;
	char *filter_sql = "";
	char *where_rec = NULL;
...
textlist_p db_sql_get_dirty_by_peer(db_conn_p db, const char *myhostname,
		peername_p peername) {
	const char *patlist = "/";
	return db_sql_get_dirty_by_peer_match(db, myhostname, peername, 1, &patlist,
			1, NULL);
}

textlist_p db_sql_get_old_operation(db_conn_p db, const char *checktxt,
		peername_p peername, filename_p filename, const char *device,
		const char *ino, BUF_P buffer) {
	// unused
	(void) buffer;
...
int db_sql_add_dirty(db_conn_p db, const char *file_new, int new_force,
		const char *myhostname, peername_p peername, const char *op_str,
		const char *checktxt, const char *dev, const char *ino,
		const char *result_other, operation_t op, int mode, int mtime) {
	...
	SQL(db, "Marking File Dirty",
			...
			db_escape(db, file_new), new_force ? "1" : "0",
			db_escape(db, myhostname), db_escape(db, peername),
			op_str, // csync_mode_op_str(mode, op),
			db_escape(db, checktxt), (dev ? dev : "NULL"), (ino ? ino : "NULL"),
			result_enc, op, mode, get_file_type(mode), mtime);
	//db->free(db, encoded);
	buffer_destroy(buf);
	return 0;
}
...
int db_sql_update_dev_no(db_conn_p db, filename_p encoded, int recursive,
		dev_t old_no, dev_t new_no) {
	char *where_rec = csync_generate_recursive_sql(encoded, recursive, 0, 0);
	SQL(db, "Update device no",
			"UPDATE file set device=%lu WHERE %s AND device = %lu", new_no,
			where_rec, old_no);
	free(where_rec);
	return 0;
}

int db_sql_update_file(db_conn_p db, filename_p encoded,
		const char *checktxt_encoded, struct stat *file_stat,
		const char *digest) {
	BUF_P buf = buffer_init();
	int count =
			SQL(db, "Update file entry",
					"UPDATE file set checktxt='%s', device=%lu, inode=%llu, "
							"                digest=%s, mode=%u, mtime=%lu, size=%lu, type=%u where filename = '%s'",
					checktxt_encoded, fstat_dev(file_stat), file_stat->st_ino,
					buffer_quote(buf, digest), (07777777 & file_stat->st_mode),
					file_stat->st_mtime, file_stat->st_size,
					get_file_type(file_stat->st_mode), encoded);

	buffer_destroy(buf);
	return count;
}

int db_sql_insert_file(db_conn_p db, filename_p encoded,
		const char *checktxt_encoded, struct stat *file_stat,
		const char *digest) {
	BUF_P buf = buffer_init();
	int count =
			SQL(db, "Adding new file entry",
					"INSERT INTO file (hostname, filename, checktxt, device, inode, digest, mode, size, mtime, type) "
							"values ('%s', '%s', '%s', %lu, %llu, %s, %u, %lu, %lu, %u) ",
					myhostname, encoded, checktxt_encoded, fstat_dev(file_stat),
					file_stat->st_ino, buffer_quote(buf, digest),
...
int db_sql_insert_update_file(db_conn_p db, filename_p encoded,
		const char *checktxt_encoded, struct stat *file_stat,
		const char *digest) {
	int count = db_sql_update_file(db, encoded, checktxt_encoded, file_stat,
			digest);
	if (count <= 0)
		count = db_sql_insert_file(db, encoded, checktxt_encoded, file_stat,
				digest);
	return count;
}

int filter_child_to_deleted_dir(const char *filename, const char *checktxt,
		char **last_dir_del) {
	return 0;
	...
	if (strstr(checktxt, "type=dir")) {
		...
		*last_dir_del = (char *) malloc(len + 1);
		...
		if (*last_dir_del != NULL
				&& strstr(filename, *last_dir_del) == filename) {
...
int db_sql_check_delete(db_conn_p db, const char *file, int recursive,
		int init_run) {
	...
	char *where_rec = csync_generate_recursive_sql(file_encoded, recursive, 0,
			1);
	csync_log(LOG_DEBUG, 3, "file %s encoded %s. Hostname: %s \n", file,
			file_encoded, myhostname);
...
{			filename_p filename = db_decode(SQL_V(0));
	...
	for (t = tl; t != 0; t = t->next) {
		if (!init_run) {
			...
			csync_mark(db, t->value, 0, 0, OP_RM, t->value2, t->value3,
					t->value4, t->intvalue, now);
			count_deletes++;
		}
		SQL(db, "Delete file from DB. It isn't with us anymore.",
				"delete from file WHERE hostname = '%s' AND filename = '%s'",
				myhostname, db_escape(db, t->value));
	}
...
int db_sql_move_file_to_dirty(db_conn_p db, const char *file, int recursive,
		const char *peername) {
	const char *file_encoded = db_escape(db, file);
	char *where_rec = csync_generate_recursive_sql(file_encoded, recursive, 0,
			1);
...
int db_sql_add_action(db_conn_p db, filename_p filename, const char *prefix_cmd,
		const char *logfile) {
	SQL(db, "Add action to database",
...
int db_sql_del_action(db_conn_p db, filename_p filename, const char *prefix_cmd) {
	SQL(db, "Del action before insert",
			"DELETE FROM action WHERE filename='%s' AND command='%s' ",
			db_escape(db, filename), db_escape(db, prefix_cmd));
	return 0;
}

int db_sql_remove_action_entry(db_conn_p db, filename_p filename,
		const char *command, const char *logfile) {
	SQL(db, "Remove action entry", "DELETE FROM action WHERE command = '%s' "
			"and logfile = '%s' and filename = '%s'", command, logfile,
			db_escape(db, filename));
	return 0;
}

textlist_p db_sql_check_file_same_dev_inode(db_conn_p db, filename_p filename,
...
textlist_p db_sql_check_dirty_file_same_dev_inode(db_conn_p db,
		peername_p peername, filename_p filename, const char *checktxt,
	...
	const char *sqls[] =
			{
					" SELECT filename, checktxt, digest, operation FROM dirty WHERE myname = '%s'"
...
int db_sql_init(db_conn_p conn) {
	...
	conn->check_dirty_file_same_dev_inode =
			db_sql_check_dirty_file_same_dev_inode;
