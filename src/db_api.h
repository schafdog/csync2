...
typedef textlist_p (*check_old_operation_f)(const char *file, int mode,
		struct stat *st_file, const char *old_filename, const char *old_other,
...
struct db_conn_t {
	...
	void (*shutdown)();
	void (*mark)(db_conn_p conn, char *active_peerlist, const char *realname,
			int recursive);
	...
	// query functions 
	int (*list_dirty)(db_conn_p conn, char **active_peers, const char *realname,
			int recursive);
	void (*list_hint)(db_conn_p conn);
	void (*list_files)(db_conn_p conn, filename_p filename);
	textlist_p (*list_file)(db_conn_p conn, filename_p filename,
			const char *myname, peername_p peername, int recursive);
	void (*list_sync)(db_conn_p conn, peername_p myname, peername_p peername);

	int (*is_dirty)(db_conn_p conn, filename_p filename, peername_p peername,
			int *operation, int *mode);
	void (*force)(db_conn_p conn, const char *realname, int recursive);
	int (*upgrade_db)(db_conn_p conn);
	int (*update_format_v1_v2)(db_conn_p conn, filename_p filename,
			int recursive, int do_it);
	...
	textlist_p (*find_dirty)(db_conn_p conn,
			int (*filter_dirty)(filename_p filename, const char *localname,
					peername_p peername));
	textlist_p (*find_file)(db_conn_p conn, filename_p pattern,
			int (*filter_file)(filename_p filename));
	int (*add_dirty)(db_conn_p conn, const char *file_new, int csync_new_force,
			const char *myname, peername_p peername, const char *operation,
			const char *checktxt, const char *dev, const char *ino,
			const char *result_other, int op, int mode, int mtime);

	void (*remove_dirty)(db_conn_p conn, peername_p peername,
			filename_p filename, int recursive);

	textlist_p (*get_dirty_by_peer_match)(db_conn_p db, const char *myname,
			peername_p peername, int recursive, const char *patlist[], int num,
			int (*match_func)(const char *file, filename_p pattern,
					int recursive));

//    void        (*clear_dirty)     (db_conn_p conn, peername_p peername, filename_p filename, int recursive);
	void (*clear_operation)(db_conn_p conn, const char *myname,
	...
	textlist_p (*get_command_filename)(db_conn_p conn, filename_p filename,
			const char *logfile);
	textlist_p (*get_hosts)(db_conn_p conn);
	textlist_p (*get_hints)(db_conn_p conn);

	int (*update_file)(db_conn_p conn, filename_p encoded,
			const char *checktxt_encoded, struct stat *file_stat,
			const char *digest);
	int (*insert_file)(db_conn_p conn, filename_p encoded,
			const char *checktxt_encoded, struct stat *file_stat,
			const char *digest);
	int (*insert_update_file)(db_conn_p conn, filename_p encoded,
			const char *checktxt_encoded, struct stat *file_stat,
			const char *digest);
	int (*update_dev_no)(db_conn_p db, filename_p encoded, int recursive,
			dev_t old_no, dev_t new_no);
	int (*check_delete)(db_conn_p conn, filename_p filename, int recursive,
			int init_run);

	int (*del_action)(db_conn_p conn, filename_p filename,
			const char *prefix_command);
	int (*add_action)(db_conn_p conn, filename_p filename,
			const char *prefix_command, const char *logfile);
	int (*remove_action_entry)(db_conn_p conn, filename_p filename,
			const char *command, const char *logfile);

	int (*check_file)(db_conn_p db, const char *file, const char *enc,
			char **other, char *checktxt, struct stat *file_stat, BUF_P buffer,
			int *operation, char **digest, int flags, dev_t *old_no);

	textlist_p (*check_file_same_dev_inode)(db_conn_p db, filename_p filename,
			const char *checktxt, const char *digest, struct stat *st);
	textlist_p (*check_dirty_file_same_dev_inode)(db_conn_p db,
...
int db_exec2(db_conn_p conn, const char *exec,
		void (*callback)(void*, int, int), void *data, const char **err);

int db_prepare_stmt(db_conn_p conn, const char *statement, db_stmt_p *stmt,
		char **value);
