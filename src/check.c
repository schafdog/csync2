...
textlist_p check_old_operation(const char *file, operation_t operation, int mode, struct stat *st_file,
		const char *other, const char *old_filename, const char *old_other, operation_t old_operation,
		const char *old_checktxt, peername_p peername, BUF_P buffer) {
	...
	// NEW/MK A -> RM A => remove from dirty, as it newer happened if it is same filename
			...
			&& (old_operation == OP_NEW || old_operation == OP_HARDLINK || old_operation == OP_MKDIR)
			&& !strcmp(file, old_filename)) {
	...
	// NEW/MK A -> MOD (still NEW)
		...
		dirty = 1;
			...
			&& ((OP_RM == old_operation || OP_MOVE == old_operation || OP_UNDEF == old_operation) && OP_NEW == operation)
...
void csync_mark_other(db_conn_p db, filename_p file, const char *thispeer, const char *peerfilter,
		operation_t operation_org, const char *checktxt, const char *dev, const char *ino, const char *org_other,
		int mode, int mtime) {
	...
	for (pl_idx = 0; pl[pl_idx].peername; pl_idx++) {
		...
		if (!peerfilter || !strcmp(peerfilter, peername)) {
			csync_info(1, "mark other operation: '%s' '%s:%s' '%s'.\n",
					csync_mode_op_str(rc_file ? 0 : st_file.st_mode, operation), peername, file, (other ? other : "-"));
			...
			if (dirty)
				db->add_dirty(db, file_new, 0, myname, peername, csync_operation_str(operation), checktxt, dev, ino,
...
void csync_mark(db_conn_p db, const char *file, const char *thispeer, const char *peerfilter, operation_t operation,
...
textlist_p csync_check_file_same_dev_inode(db_conn_p db, filename_p filename, const char *checktxt, const char *digest,
		struct stat *st) {
	textlist_p tl = 0;
...
textlist_p csync_check_move(db_conn_p db, peername_p peername, filename_p filename, const char *checktxt,
...
textlist_p csync_check_link_move(db_conn_p db, peername_p peername, filename_p filename, const char *checktxt,
		int operation, const char *digest, struct stat *st, textlist_loop_t loop) {
	if (loop) {
	...
	if (n < 0) {
		...
		while (n--) {
			...
			if (strcmp(namelist[n]->d_name, ".") && strcmp(namelist[n]->d_name, "..")) {
				...
				sprintf(fn, "%s/%s", !strcmp(directory, "/") ? "" : directory, namelist[n]->d_name);
...
int csync_check_file_mod(db_conn_p db, const char *file, struct stat *file_stat, int flags) {
	...
	char *checktxt = buffer_strdup(buffer,
			csync_genchecktxt_version(file_stat, file, SET_USER | SET_GROUP, db->version));
