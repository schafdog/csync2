...
void example_basic_usage(const char *connect) {
    std::cout << "=== Basic Database Usage Example ===" << std::endl;

    // Create a database connection
    auto db = DatabaseFactory::create_from_url(connect);
    if (!db) {
    ...
    stmt->bind(1, checksum);
    stmt->bind(2, 999);
    stmt->bind(3, 1640995200);
...
void example_query_usage(const char *connect) {
...
void example_transaction_usage(const char *connect) {
...
int main() {
    ...
    try {
        //example_basic_usage("sqlite:test.db");
        auto db_url = "mysql:localhost/test";
        example_basic_usage(db_url);
        example_query_usage(db_url);
        example_transaction_usage(db_url);
