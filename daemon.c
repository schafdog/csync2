...
struct textlist *csync_hardlink(const char *filename, struct stat *st, struct textlist *tl)
{
  ...
  while (t) {
    char *src  = t->value;
    int rc = unlink(src);
    csync_daemon_hardlink(filename, src, &cmd_error);
    t = t->next;
  }
  textlist_free(tl);
  return 0;
			  ...
			  const char **cmd_error)
{
  ...
  case A_MKLINK:
    ...
    break;
  case A_MKHLINK:
    return csync_daemon_hardlink(filename, prefixsubst(secondfile), cmd_error);
    break;
  case A_MV:
...
void csync_daemon_session(int db_version, int protocol_version)
{
  ...
  while ( conn_gets(line, 4096) ) {
    ...
    if (!cmd->text) {
      ...
      csync_end_command(tag[2], tag, cmd_error);
      continue; 
    }
